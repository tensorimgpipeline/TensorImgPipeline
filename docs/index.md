...
The TensorImgPipeline is a python package, which allows the developer to create new Deep Learning approaches as a generalized Pipeline.

Most approaches to new experiments are script or notebook based.
...
The script approach often leads to an unmaintainable code structure.
Often multiple things are done in one place, which logically are only weakly connected.
...
We might need the structure, because they were a direct execution of our thoughts, which also just came up while creating the first prototype of the experiment.
But melting this chaos into a more structured and organized environment often seems like a huge effort.
...
After that, the [Getting Started](getting_started.md) provides the information needed to build the structured Pipeline.
